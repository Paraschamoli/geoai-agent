...
check: ## Run code quality tools
	...
	@uv run mypy . || echo "⚠️  mypy not available, skipping type checks"
...
run: env-check ## Run the agent locally
	@echo "🤖 Starting geoai-agent..."
	@uv run python -m geoai_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting geoai-agent in development mode..."
	@uv run watchfiles "python -m geoai_agent" geoai_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t geoai_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t geoai_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi geoai_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag geoai_agent:latest para5/geoai_agent:latest
	@docker push para5/geoai_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep geoai_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep geoai_agent || echo "No images found"
	@echo "\nAgent Health:"
